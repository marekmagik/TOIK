 ...
 * pl.edu.agh.araucaria.gui.visualisation.standard.FullSizePanel.java
 *
        ...
        for (int i = 0; i < refutationList.size(); i++) {
            ...
            if (invertedTree) {
                vertex.setDrawPoint((xMin + xMax) / 2, canvasHeight -
                        topOffset - height - layerNum * edgeHeight);
            } else {
                vertex.setDrawPoint((xMin + xMax) / 2, topOffset + height + layerNum * edgeHeight);
            }
