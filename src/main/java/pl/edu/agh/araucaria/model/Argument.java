 ...
 * pl.edu.agh.araucaria.model.Argument.java
 *
...
import pl.edu.agh.araucaria.history.HiddenInfo;
...
public class Argument {
     ...
     */
    public static final String WigmoreIDPrefix = "Wigmore_";
    ...
    /**
     * Creates a new instance of pl.edu.agh.araucaria.model.Argument
     */
    public Argument() {
        try {
    ...
    public void setTopWigmoreIndex(int topWigmoreIndex) {
        this.topWigmoreIndex = topWigmoreIndex;
    }
    ...
    public Vector ownerExists(String name, int column) {
        for (Object anOwnerList : ownerList) {
            Vector row = (Vector) anOwnerList;
    ...
    public void addXMLSubtrees() {
        ...
        for (int i = 0; i < oldSubtreeList.size(); i++) {
            ...
            subtree.selectEdges();
            try {
                addSubtree(subtree.getArgumentType());
            } catch (SubtreeException ignored) {
            }
            clearAllSelections();
        }
    }
    ...
    public boolean clearAllSelections() {
        ...
        if (treeTraversal == null) return false;
     ...
     * file.
     */
    public void addSubtree(ArgType argType) throws SubtreeException {
        ...
        while (edgeList.hasMoreElements()) {
            ...
            if (!source.getVisited()) {
                if (source.getLayer() == highestLayer) {
                    ...
                    premiseList.add(source.getLabel());
                }
                source.setVisited(true);
            }
            if (!dest.getVisited()) {
                premiseList.add(dest.getLabel());
    ...
    public String incrementID(String id) {
        ...
        if (labelLength == 1) {
            ...
            if (label[1] == 'Z') {
                ...
                if (label[0] == 'Z') {
                    label[0] = 'a';
                } else if (label[0] == 'z') { // We've used up all the IDs
                    return null;
                } else {
                    label[0]++;
                }
    ...
    private TreeVertex freeListContainsVertexID(String id) {
        for (int i = 0; i < freeVertexList.size(); i++) {
            ...
            if (vertex.getShortLabelString().equals(id)) {
                return vertex;
            }
        }
        return null;
    ...
    public void addFreeVertex(String label, GeneralPath shape, int offset, DiagramBase diagram) {
        ...
        if (!diagram.getClass().getName().contains("Wigmore")) {
     ...
     */
    public Vector<HiddenInfo> getHiddenList() {
        Vector<HiddenInfo> visible = new Vector<>();
        for (int i = 0; i < getTree().getVertexList().size(); i++) {
            TreeVertex vertex = (TreeVertex) getTree().getVertexList().elementAt(i);
            visible.add(new HiddenInfo(vertex.getIsHiddenTable(), vertex.getShortLabelString()));
        }
        return visible;
    }
    ...
    public boolean doSaveXMLFile(File currentOpenXMLFile) {
        try {
            FileOutputStream chosenOutput = new FileOutputStream(currentOpenXMLFile);
    ...
    public String writeXML() {
        try {
            ...
            root.appendChild(doc.createProcessingInstruction("main.java.Araucaria", encoding));
            // Schemeset
            ...
            // pl.edu.agh.araucaria.model.Argument tree
    ...
    public int selectAddedNodes() {
        ...
        for (int i = 0; i < vertexList.size(); i++) {
            ...
            // an added node
            if (vertex.isSelected()) {
                String toulminRole = vertex.roles.get("toulmin");
                if (toulminRole.equals("rebuttal")) {
                    TreeVertex parent = vertex.getParent();
                    String addedNegation = parent.roles.get("addedNegation");
    ...
    public void standardToWigmore() {
        ...
        for (int i = 0; i < vertexList.size(); i++) {
            ...
            if (vertex.isRefutation() && !(vertex.roles.get("wigmore")).contains("evidence")) {
                    ...
                    if (!dest.isVirtual()) {
                        if (dest.roles.get("addedNegation") != null &&
                            ...
                            if (dest.getEdgeList().size() > 1) {
                                dest.getIsHiddenTable().put("wigmore", "false");
                            }
                            if (dest.getIsHiddenTable().get("wigmore").equals("true")) {
                            ...
                            if (!(vertex.roles.get("addedNegation") != null && vertex.roles.get("addedNegation").equals("yes") &&
                                    vertex.getIsHiddenTable().get("wigmore").equals("true"))) {
                                addWigmoreEdge(vertex, dest, edge);
                            }
                        }
                    } else {  // Handle LAs
    ...
    private void addWigmoreEdge(TreeVertex vertex, TreeVertex dest, TreeEdge edge) {
        String destRole = dest.roles.get("wigmore");
        if (destRole.contains("evidence")) {
            vertex.wigmoreEvidenceEdges.add(edge);
        } else if (destRole.contains("explanatory")) {
            vertex.wigmoreExplanatoryEdges.add(edge);
        } else if (destRole.contains("corroborative")) {
    ...
    public void standardToToulmin() {
        ...
        // create a pl.edu.agh.araucaria.model.TreeVertex to represent them in Toulmin.
        for (int i = 0; i < vertexList.size(); i++) {
            TreeVertex vertex = (TreeVertex) vertexList.elementAt(i);
    ...
    public String getMaxShortLabel() {
        ...
        for (int i = 0; i < freeVertexList.size(); i++) {
            TreeVertex vertex = (TreeVertex) freeVertexList.elementAt(i);
            String label = vertex.getShortLabelString();
...
//            currentParent);
                ...
                else if (parentVertex != currentParent) {  // Error: parents not all the same
    ...
    public void deleteOwners(Vector remList, boolean selectedOnly) {
        ...
        while (nodeList.hasMoreElements()) {
            ...
            for (int i = 0; i < remList.size(); i++) {
                vertex.getOwners().remove(remList.elementAt(i));
            }
        }
    }

    /**
     * Returns a Vector of pl.edu.agh.araucaria.model.TreeVertex containing all vertices selected
     */
    ...
    /**
     * Returns a Vector of pl.edu.agh.araucaria.model.TreeEdge containing all selected edges
     */
        ...
        if (checkList.size() == 1) {
            ...
            if (dest.isVirtual()) {
                for (int i = 0; i < dest.getEdgeList().size(); i++) {
                    ...
                    if (edge.isSelected()) edgeList.add(edge);
                }
            }
        }
    }
    ...
    public Vector getSelectedWigmoreEdges() {
        Vector<TreeEdge> edgeList = new Vector<>();
    ...
    public boolean supportLabelExists(String name) {
        for (Object aSupportLabelList : supportLabelList) {
            String label = (String) aSupportLabelList;
