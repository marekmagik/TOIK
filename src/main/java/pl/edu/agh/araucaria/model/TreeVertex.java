...
/**
 * Extends the pl.edu.agh.araucaria.model.Vertex class to represent nodes in a tree
 */
public class TreeVertex extends Vertex implements Serializable {
    ...
    public Hashtable<String, String> roles = new Hashtable<>();   // Stores roles for non-standard diagram types
    // Toulmin stuff
    ...
    // Wigmore stuff
    public Vector<TreeEdge> wigmoreExplanatoryEdges = new Vector<>();
    public Vector<TreeEdge> wigmoreCorroborativeEdges = new Vector<>();
    public Vector<TreeEdge> wigmoreEvidenceEdges = new Vector<>();
    ...
    private String shortLabel;        // Single character label
    ...
    private Point m_drawPoint;        // Point where the vertex should be drawn
    private Point m_drawPointFullText;  // Point for drawing the vertex in FullTextDialog
    ...
    private Hashtable isHiddenTable = new Hashtable(); // Is vertex displayed in a given diagram?
    ...
    boolean m_selected;        // pl.edu.agh.araucaria.model.Vertex is selected (highlighted)
    boolean dummy = false;        // pl.edu.agh.araucaria.model.Vertex is the dummy root in a split tree
    boolean m_linkedStemSelected; // If this is a virtual vertex, true if edge leading to vertex is selected.
    private Object m_auxObject;      // Auxiliary object associated with vertex
    ...
    public void initRoles() {
        roles.put("toulmin", "data");
        getIsHiddenTable().put("toulmin", "false");
        getIsHiddenTable().put("wigmore", "true");
        roles.put("addedNegation", "no");
        if (!isVirtual())
            roles.put("wigmore", "evidenceTestAffirm");
        else
    ...
    /**
     * Adds an existing pl.edu.agh.araucaria.model.TreeEdge - used in undo
     */
    public void addEdge(TreeEdge edge, int order) {
        try {
            getEdgeList().add(order, edge);
    ...
    public boolean hasAddedNegationNoSupport() {
        return getParent() != null
                && getParent().roles.get("addedNegation") != null
                && getParent().roles.get("addedNegation").equals("yes")
                && getParent().getEdgeList().size() == 1;
    }
    ...
    public TreeEdge edgeExists(TreeVertex destVertex) {
        Enumeration edgeList = getEdgeList().elements();
    ...
    public boolean isWigmoreLeaf() {
        return wigmoreCorroborativeEdges.size() <= 0
                && wigmoreEvidenceEdges.size() <= 0
                && wigmoreExplanatoryEdges.size() <= 0;
    }
    ...
    /**
     * Returns pl.edu.agh.araucaria.model.TreeEdge with 'this' as source vertex and
     * 'vertex' as dest vertex; null if not found.
     */
    public TreeEdge getEdge(TreeVertex vertex) {
        return getEdge(getEdgeList(), vertex);
    }
    ...
    public void refutationToRebuttal() {
        ...
        if (parent.roles.get("addedNegation") != null &&
            ...
            parent.getIsHiddenTable().put("toulmin", "true");
        } else {
            parent.getIsHiddenTable().put("toulmin", "false");
        }
    ...
    public boolean putRefutationFirst() {
        if (this.getNumRefutations() == 0 ||
                ((TreeEdge) getEdgeList().elementAt(0)).getDestVertex().isRefutation())
            return false;
        TreeEdge edge0 = (TreeEdge) getEdgeList().elementAt(0);
        for (int i = 1; i < getEdgeList().size(); i++) {
            if (((TreeEdge) getEdgeList().elementAt(i)).getDestVertex().isRefutation()) {
                getEdgeList().set(0, getEdgeList().elementAt(i));
                getEdgeList().set(i, edge0);
                return true;
            }
        }
        return false;
    ...
    public TreeVertex getFirstChild() {
        if (getEdgeList().size() == 0)
            return null;
        return ((TreeEdge) getEdgeList().elementAt(0)).getDestVertex();
    }
    ...
    public void getRefutationList(Vector refutList) {
        for (int i = 0; i < getEdgeList().size(); i++) {
            TreeVertex vertex = ((TreeEdge) getEdgeList().elementAt(i)).getDestVertex();
    ...
    public void setIsHiddenTable(Hashtable isHiddenTable) {
        this.isHiddenTable = isHiddenTable;
    }

    @Override
    public String toString() {
        return (String) m_label;
    }
}