...
/*
 * pl.edu.agh.araucaria.wigmore.WigmoreFullSizePanel.java
 *
...
public class WigmoreFullSizePanel extends WigmoreFullTextPanel {
    ...
    /**
     * Creates a new instance of pl.edu.agh.araucaria.wigmore.WigmoreFullSizePanel
     */
    ...
    public void drawVertex(Graphics2D gg, TreeVertex vertex, Color color) {
        ...
        int offset = 0;
        if (wigmoreRole.toLowerCase().contains("explan")) {
            position = WigmoreImages.NodePosition.LEFT;
            offset = -(int) bound.getWidth() / 4;
        } else if (wigmoreRole.toLowerCase().contains("corrob")) {
    ...
    public void calcTextLayout(TreeVertex vertex) {
        ...
        String text = vertex.getShortLabelString();
        vertex.textLayout = calcLayout(text, DiagramBase.transparentMap, 20);
